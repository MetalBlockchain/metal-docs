...
All configuration options and their default values are described [here](/nodes/configure/metalgo-config-flags.md).
...
`$HOME/.metalgo/configs/chains/C/config.json`. You can also tell AvalancheGo
...
header field in Avalanche C-Chain blocks, which is used move METAL between chains
