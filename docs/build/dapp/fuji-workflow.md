...
4. Get METAL from the Fuji faucet
...
We can get a "drip" of METAL from the Fuji faucet. If you already have
an METAL balance greater than zero on Mainnet, 
...
These METAL are for the Fuji Testnet and have no monetary value.
...
const main = async (): Promise<any> => {
  provider.getBalance(address).then((balance) => {
    ...
    console.log(`balance: ${balanceInAvax} METAL`);
    // balance: 2 METAL
  });
};
...
We'll see that it has the METAL balance and
...
The [Fuji Faucet](https://faucet.avax.network) sends METAL to X-Chain or C-Chain
