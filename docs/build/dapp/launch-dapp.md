...
[GitHub](https://github.com/MetalBlockchain/metalgo/releases). If you're going to
...
scripts](https://github.com/MetalBlockchain/metalgo/tree/master/scripts) in the
AvalancheGo repo for various Docker configs.

### Node Configuration

Node configuration options are explained
[here](/nodes/configure/metalgo-config-flags.md). But unless you have
...
`$HOME/.metalgo/configs/chains/C/config.json`. We will go over how to adjust
...
`~/.metalgo/db/`) and start the node again.
