...
[Run a Metal Blockchain Node Manually](/nodes/run/node-manually.md#run-an-avalanche-node-from-source)
...
[it only needs 1 METAL to become a validator on the Fuji Testnet](/nodes/validate/what-is-staking.md)
and you can get the test token from the [faucet](https://faucet.avax.network/). If you already have an METAL
...
To deploy the Subnet, you will need some testnet METAL on the P-chain.
...
```bash
✔ Path to your avalanchego plugin dir (likely avalanchego/build/plugins): /home/user/go/src/github.com/MetalBlockchain/metalgo/build/plugins
```
