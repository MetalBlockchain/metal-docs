# Using Hardhat with the Metal C-Chain

## Introduction

Metal Blockchain is an open-source platform for launching decentralized applications
and enterprise blockchain deployments in one interoperable, highly scalable
ecosystem. Metal Blockchain gives you complete control on both the network and
application layers—helping you build anything you can imagine.

The Metal Network is composed of many blockchains. One of these blockchains
is the C-Chain (Contract Chain), which is an Ethereum Virtual Machine instance.
The C-Chain's API is almost identical to an Ethereum node's API. Metal Blockchain
...
and deployment of smart contracts to Metal Blockchain's C-Chain. We'll be building
...
repository](https://github.com/MetalBlockchain/metal-smart-contract-quickstart) and
install the necessary packages via `yarn`.

```text
git clone https://github.com/MetalBlockchain/metal-smart-contract-quickstart.git
...
[hardhat.config.ts](https://github.com/MetalBlockchain/metal-smart-contract-quickstart/blob/main/hardhat.config.ts).
This file configures necessary network information to provide smooth interaction
with Metal MetalBlockchain. There are also some pre-defined private keys for testing on a
local test network.
...
scripts in
[package.json](https://github.com/MetalBlockchain/metal-smart-contract-quickstart/blob/main/package.json).
...
Prints a list of accounts on the local Metal Network Runner network.
...
Prints a list of accounts and their corresponding METAL balances on the local
Metal Network Runner network.
...
## Typical Metal Network Runner Workflow

### Run Metal Network Runner

First confirm you have the latest MetalGo built.
...
Confirm you have Metal Network Runner installed by following the steps listed [here](../../quickstart/create-a-local-test-network.md#installation)

Start Metal Network Runner and run a script to start a new local network.
...
Now you're running a local Metal network with 5 nodes.

### Fund Accounts

Transfer 1,000 METAL from the X-Chain to each of the 10 accounts in
`hardhat.config.ts` with the script
[`fund-cchain-addresses`](https://github.com/MetalBlockchain/metal-smart-contract-quickstart/blob/main/scripts/fund-cchain-addresses.js).
...
```text
$ cd /path/to/metal-smart-contract-quickstart
$ yarn fund-cchain-addresses
yarn run v1.22.4
npx hardhat run scripts/fund-cchain-addresses.js
Exporting 1000 METAL to each address on the C-Chain...
2b75ae74ScLkWe5GVFTYJoP2EniMywkcZySQUoFGN2EJLiPDgp
Importing METAL to the C-Chain...
2dyXcQGiCk1ckCX4Fs8nLgL8GJgsM72f9Ga13rX5v9TAguVJYM
✨  Done in 5.03s.
```

Confirm each of the accounts are funded with 1000 METAL.
...
Send each of the accounts some METAL from the first account.

```text
$ yarn send-metal-wallet-signer --network local
yarn run v1.22.4
npx hardhat run scripts/sendMetalWalletSigner.ts --network local
Seeding addresses with METAL
✨  Done in 1.33s.
```
...
`--network` flag with `local` (for example a local network created with Metal
Network Runner), `tahoe`, or `mainnet` for each respective environment. If you
...
[ERC20.sol](https://github.com/MetalBlockchain/metal-smart-contract-quickstart/blob/main/contracts/ERC20.sol)
...
is accepted. However for other networks like `tahoe` or `mainnet` you need to
...
Now you have the tools you need to launch a local Metal Blockchain network, create a
Hardhat project, as well as create, compile, deploy and interact with Solidity
contracts.

If you have questions, problems, or just want to chat with us, you can reach us on our public [Telegram](https://t.me/metaldevelopers) chat. We'd love to hear from you and find out what you're building on Metal Blockchain!
