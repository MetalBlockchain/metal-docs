# Using Truffle with the Metal C-Chain
...
Metal Blockchain's C-Chain, which is an instance of the EVM.
...
- [Metal Network
  Runner](https://github.com/MetalBlockchain/metal-network-runner) is a tool for
  running a local Metal network. It's similar to Truffle's
  [Ganache](https://www.trufflesuite.com/ganache).
- [NodeJS](https://nodejs.org/en) v8.9.4 or later.
- Truffle, which you can install with `npm install -g truffle`

## Start up a Local Metal Network
...
A five node Metal network is running on your machine. Network will run until you Ctrl + C to exit.
...
API calls to a different MetalGo node. Also note that we're setting the
`gasPrice` and `gas` to the appropriate values for the Metal C-Chain.
...
[here](https://raw.githubusercontent.com/MetalBlockchain/metal-docs/master/scripts/fund-cchain-addresses.js).
You can also download it using this command:

```text
wget -nd -m https://raw.githubusercontent.com/MetalBlockchain/metal-docs/master/scripts/fund-cchain-addresses.js
```
...
Script will fund 1000 METAL to each account in `accounts` list above. After
...
[tutorial](https://metalstaking.com/how-to-transfer-cross-chain-in-the-metal-wallet).
You'll need to send at least `135422040` METAL to the account to cover the cost
...
`C-chain`/`Coreth` configs. The Metal Network Runner currently does not
