...
```bash
./scripts/build.sh ~/.metalgo/plugins/mDV3QWRXfwgKUWb9sggkv4vQxAQR4y2CyKrt5pLZ5SzQ7EHBv
```
...
[this](/nodes/configure/metalgo-config-flags.md#config-file) for more info on configuration
file and flags.
...
```bash
./build/avalanchego --config-file ~/.metalgo/config.json
```
