...
`$HOME/.metalgo/configs/chains`, then `config.json` should be placed at
`$HOME/.metalgo/configs/chains/C/config.json`.
...
Global transaction fee (price \* `gaslimit`) cap (measured in METAL) for
