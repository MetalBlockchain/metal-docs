...
pagination_label: Run a Metal Blockchain Node with Amazon Web Services
sidebar_position: 0
---

# Run a Metal Blockchain Node with Amazon Web Services (AWS)
...
- As the staked amount increases, choose larger instance sizes to accommodate the additional workload. For every order of magnitude increase in stake, move up one instance size. For example, for a 20k METAL stake, a 4xlarge instance is suitable.
...
space size](https://github.com/MetalBlockchain/metalgo#installation) before
...
```text
scp -r ubuntu@PUBLICIP:/home/ubuntu/.metalgo/staking ~/aws_avalanche_backup
```
