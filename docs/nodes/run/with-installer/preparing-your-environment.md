...
# Run a Metal Blockchain Node Using the Install Script
...
Hardware requirements shall scale with the amount of METAL staked on
the node and/or network activity. Nodes with big stakes (100k+ METAL) will need
...
node files stored in `$HOME/.metalgo` directory. Those files will not be
...
`$HOME/.metalgo/staking` and store them somewhere secure. You can use those
