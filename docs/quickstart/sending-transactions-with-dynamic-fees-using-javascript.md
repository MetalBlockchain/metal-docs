...
- Basic familiarity with [the Metal
  C-Chain](../apis/metalgo/apis/c-chain.md) network and [EVM
...
Using the HTTP provider, we will connect to one of the nodes on the Tahoe
...
const nodeURL = "https://tahoe.metalblockchain.org/ext/bc/C/rpc"
...
const chainId = 381932
const avalanche = new Avalanche(
  "tahoe.metalblockchain.org",
  undefined,
  "https",
  chainId
)
...
fee and max priority fee per gas in units of `nMETAL` or `gwei` (1 METAL = 10^9
gwei).
...
one-billionth of a billionth of `METAL` (1 METAL = 10^18 wei).
...
on the console, the URL for transaction status on the Metalscan explorer.
...
nMETAL per gas for a transaction and a small tip of 2 nMETAL, then we will call
...
```bash
View transaction with nonce 25: https://tahoe.metalscan.io/tx/0xd5b92b85beaf283fbaeeefb95c9a17a6b346a05b6f9687f2d6e421aa79243b35
```
...
fee parameters to the C-Chain of the Metal Blockchain network using JavaScript. It also
