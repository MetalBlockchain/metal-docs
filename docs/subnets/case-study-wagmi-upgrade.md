...
This case study uses [WAGMI](https://wagmi.metalscan.io) Subnet upgrade to show how a
...
[Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm) aims to provide an easy to use toolbox to
...
v0.2.8](https://github.com/MetalBlockchain/subnet-evm/releases/tag/v0.2.8) introduces the long awaited
feature to perform network upgrades by just using a few lines of JSON in a configuration file.

### WAGMI

The [WAGMI](https://wagmi.metalscan.io) ("We're All Going to Make It") Subnet Demo is a
high throughput testbed for EVM optimizations. It is parameterized to run at a factor more capacity
than Tahoe/Mainnet C-Chain and will be used to experiment with release candidates before they make it
...
Subnet](https://github.com/MetalBlockchain/subnet-evm/blob/master/networks/testnet/11111/genesis.json) to
...
To prepare for the WAGMI network upgrade, we had announced the following information:

> The upgrade will activate the fee config manager, and enable smooth fee config updates in the
> future <https://docs.metalblockchain.org/subnets/customize-a-subnet#configuring-dynamic-fees>
>
...
> <https://docs.metalblockchain/subnets/customize-a-subnet#network-upgrades-enabledisable-precompiles>
>
...
When the node restarts, MetalGo reads the contents of the JSON file and passes it into
...
github.com/MetalBlockchain/subnet-evm/eth/backend.go:155: Initialised chain configuration
...
We will use [Remix](https://remix.ethereum.org) online Solidity IDE and MetaMask will do as well but you will need to [add
WAGMI](./wagmi.md#adding-wagmi-to-metamask) yourself.

First using MetaMask, we open the account as the owner `0x6f0f6DA1852857d7789f68a28bba866671f3880D`.

Then we connect MetaMask to WAGMI.
...
- [IAllowList.sol](https://github.com/MetalBlockchain/subnet-evm/blob/master/contract-examples/contracts/IAllowList.sol)
- and
  [IFeeManager.sol](https://github.com/MetalBlockchain/subnet-evm/blob/master/contract-examples/contracts/IFeeManager.sol).
...
[Subnet](https://github.com/MetalBlockchain/subnet-evm/blob/master/precompile/params.go#L33).
...
explorer](https://tahoe.metalscan.io/wagmi/block/0xad95ccf04f6a8e018ece7912939860553363cc23151a0a31ea429ba6e60ad5a3):
...
[Telegram Developer Channel](https://t.me/metaldevelopers). Or just reach out to tell us what exciting new things you have
built!
