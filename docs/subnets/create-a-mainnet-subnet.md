...
exporting them from the C-Chain to the P-Chain using [Metal's Web
...
The deployment requires running a [createSubnet transaction](../apis/metalgo/apis/p-chain.md#platformcreatesubnet)
...
Metal-CLI on the same box as the validator node.
...
`/etc/metalgo/config.json` - just make sure the tool has **write** access to the file. Or you could
...
need to copy your VM's plugin binary into MetalGo's plugin directory. This directory depends
on your MetalGo install location.
