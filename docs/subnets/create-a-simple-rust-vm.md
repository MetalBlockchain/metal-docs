...
The Metal Rust SDK is a developer toolkit composed of powerful building blocks and primitive
types. This tutorial will walk you through the creation of a simple VM known as the [TimestampVM
RS](https://github.com/MetalBlockchain/timestampvm-rs) using the Rust SDK. Each block in the TimestampVM's
...
- Bookmark and review the [metal-types](https://github.com/MetalBlockchain/metal-types-rs) GitHub
  repository specifically the traits and helpers defined in the
  [`subnet/rpc`](https://github.com/MetalBlockchain/metal-types-rs/tree/main/src/subnet/rpc) mod.
...
Completely new to creating a custom VM on Metal Blockchain? No problem please review [Introduction to
...
[snowman::block::ChainVM](https://github.com/MetalBlockchain/metal-types-rs/blob/main/src/subnet/rpc/snowman/block.rs)
...
- [ChainVM
  GoDoc](https://pkg.go.dev/github.com/MetalBlockchain/metalgo/snow/engine/snowman/block#ChainVm)
- [Metal Proto Docs](https://buf.build/MetalBlockchain/metal/docs/main:vm#vm.VM)
- [Snowman VMs](https://github.com/MetalBlockchain/metalgo/tree/master/vms#snowman-vms)
...
repository](https://github.com/MetalBlockchain/timestampvm-rs).
...
MetalGo searches for and registers VM plugins under the `plugins` directory of the [build
directory](../nodes/maintain/metalgo-config-flags#build-directory).
...
```bash
cp -n <path to your binary> $GOPATH/src/github.com/MetalBlockchain/metalgo/build/plugins/
```
...
[here](../nodes/maintain/metalgo-config-flags.md#vm-configs).

In this tutorial, we used the VM's ID as the executable name to simplify the process. However,
MetalGo would also accept `timestampvm-rs` or `timestamp-rs` since those are registered aliases
in previous step.
