...
This document describes how to use the new Metal-CLI to deploy a Subnet on `Tahoe`.
...
In this article, it's shown how to do the following on `Tahoe` Testnet.
...
- 1+ nodes running and fully bootstrapped on `Tahoe` Testnet. Check out the section
...
Metal can run multiple blockchains. Each blockchain is an instance of a
...
Chains. Subnet-EVM is a simplified version of [Metal C-Chain](https://github.com/MetalBlockchain/coreth).
...
[Run a Metal Node Manually](../nodes/build/run-metal-node-manually.md#connect-to-tahoe-testnet)
and this step below particularly to start your node on `Tahoe`:
...
With more data on `Tahoe`, it may take a while to bootstrap `Tahoe` Testnet from scratch.
...
If not yet installed, install `Metal-CLI` following the tutorial at [Metal-CLI installation](create-a-local-subnet.md#installation)
...
deploy one, as `Mainnet` operations requires ledger usage, while for `Tahoe` it's optional.

`Metal-CLI` supports the following key operations:
...
You should only use the private key created for this tutorial for testing operations on `Tahoe` or
...
+           +                               +-------------------------------------------------+---------------+
|           |                               | P-tahoe1a3azftqvygc4tlqsdvd82wks2u7nx85rhk6zqh  | Tahoe         |
+-----------+-------------------------------+-------------------------------------------------+---------------+
```
...
```bash
metal key create othertest --file /tmp/test.pk
Loading user key...
Key loaded
```
...
+           +                               +-------------------------------------------------+---------------+
|           |                               | P-tahoe1n5n4h99j3nx8hdrv50v8ll7aldm383na7j4j7q  | Tahoe         |
+-----------+-------------------------------+-------------------------------------------------+---------------+
| mytestkey | C-Chain (Ethereum hex format) | 0x86BB07a534ADF43786ECA5Dd34A97e3F96927e4F      | All           |
+           +-------------------------------+-------------------------------------------------+---------------+
|           | P-Chain (Bech32 format)       | P-custom1a3azftqvygc4tlqsdvd82wks2u7nx85rg7v8ta | Local Network |
...
Do these steps only to follow this tutorial for `Tahoe` addresses. To access the wallet for `Mainnet`,
...
Creating a Subnet with `Metal-CLI` for `Tahoe` works the same way as with a
...
`create` command, then first deploying to a local network and successively to `Tahoe` - and
eventually to `Mainnet`.

To create an EVM Subnet, run the `subnet create` command with a name of your choice:

```bash
metal subnet create testsubnet
```
...
```bash
metal subnet describe testsubnet
 _____       _        _ _
|  __ \     | |      (_) |
| |  | | ___| |_ __ _ _| |___
| |  | |/ _ \ __/ _  | | / __|
...
+----------------------------+----------------------------------------------------+
| Tahoe SubnetID             | XTK7AM2Pw5A4cCtQ3rTugqbeLCU9mVixML3YwwLYUJ4WXN2Kt  |
+----------------------------+----------------------------------------------------+
| Tahoe BlockchainID         | 5ce2WhnyeMELzg9UtfpCDGNwRa2AzMzRhBWfTqmFuiXPWE4TR  |
+----------------------------+----------------------------------------------------+
...
```bash
metal subnet list --deployed
go run main.go subnet list --deployed
+-------------+-------------+---------------------------------------------------+---------------+-----------------------------------------------------------------+---------+
|   SUBNET    |    CHAIN    |                       VM ID                       | LOCAL NETWORK |                          TAHOE (TESTNET)                        | MAINNET |
+-------------+-------------+---------------------------------------------------+---------------+-----------------------------------------------------------------+---------+
...
This tutorial is about deploying to `Tahoe`, so navigate with the arrow keys to `Tahoe` and hit enter.
...
[createSubnet transaction](../apis/metalgo/apis/p-chain.md#platformcreatesubnet). Therefore the
key needs to have funds.

Also, this tutorial assumes that a node is up running, fully bootstrapped on `Tahoe`, and runs
from the **same** box.

```bash
✔ Tahoe
Deploying [testsubnet] to Tahoe
Use the arrow keys to navigate: ↓ ↑ → ←
...
Make sure to provide **Tahoe P-Chain** addresses -`P-Tahoe....`-.
...
```bash
✔ Add control key
Enter P-Chain address (Ex: `P-...`): P-tahoe1vaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasz
Use the arrow keys to navigate: ↓ ↑ → ←
? Set control keys:
    Add control key
  ▸ Done
...
Well done. You have just created your own Subnet with your own Subnet-EVM running on `Tahoe`.
...
See [here](../nodes/validate/add-a-validator.md#add-a-validator-with-metal-wallet) on how to
become a validator.
...
[transaction](../apis/metalgo/apis/p-chain.md#platformaddsubnetvalidator), it's needed to tell the
...
```bash
Use the arrow keys to navigate: ↓ ↑ → ←
? Choose a network to deploy on. This command only supports Tahoe currently.:
  ▸ Tahoe
    Mainnet
```
...
This means the node is now a validator on the given Subnet on `Tahoe`!
...
`/etc/metalgo/config.json` - just make sure the tool has **write** access to the file. Or you
...
[Virtual Machine](#virtual-machine). Each VM runs its own plugin, therefore MetalGo needs to
...
```bash
✔ Path to your metalgo plugin dir (likely metalgo/build/plugins): /home/user/go/src/github.com/ava-labs/metalgo/build/plugins
```
...
```shell
✔ Path to your metalgo plugin dir (likely metalgo/build/plugins): /home/user/go/src/github.com/ava-labs/metalgo/build/plugins█
VM binary written to /home/user/go/src/github.com/ava-labs/metalgo/build/plugins/tGBrMADESojmu5Et9CpbGCrmVf9fiAJtZM5ZJ3YVDj5JTu2qw
...
For example:
./build/metalgo --network-id=Tahoe --whitelisted-subnets=2b175hLJhGdj3CzgXENso9CmwMgejaCQXhMFzBsm8hXbH2MF7H
...
including the public API server `https://tahoe.metalblockchain.org/`, to connect to MetaMask.
