...
[TimestampVM](https://github.com/MetalBlockchain/timestampvm/tree/v1.2.1). Each block in the TimestampVM's
...
repository](https://github.com/MetalBlockchain/timestampvm/tree/main).
...
methods in computer programming. MetalGo uses [Gorilla’s RPC
...
which serves our VM over gRPC so that MetalGo can call its methods.
...
This VM has a [build script](https://github.com/MetalBlockchain/timestampvm/blob/v1.2.1/scripts/build.sh)
...
`$GOPATH/src/github.com/MetalBlockchain/metalgo/build/plugins/tGas3T58KzdjLHhBDMnH2TvrddhqTji5iZAMZ3RXs2NLpSnhH`
...
JSON file at `~/.metalgo/configs/vms/aliases.json` with:
...
MetalGo searches for and registers plugins under the `plugins` directory of the
[build directory](../nodes/maintain/metalgo-config-flags#build-directory).
...
MetalGo would also accept `timestampvm` or `timestamp` since those are registered aliases in
previous step.
