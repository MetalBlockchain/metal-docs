...
**[Tahoe](../quickstart/tahoe-workflow.md)**. This bridge will help us to transfer native **WGM** coin
wrapped into **wWGM** back and forth from the WAGMI chain to the Tahoe chain. Using this guide, you
...
> WAGMI is an independent EVM-based test chain deployed on a custom Subnet on the Metal Blockchain network.

We will be using **ChainSafe**'s bridge repository, to easily set up a robust and secure bridge.

## Workflow of the Bridge

WAGMI and Tahoe chains are not interconnected by default, however, we could make them communicate.
...
[Tahoe](../quickstart/tahoe-workflow.md#set-up-tahoe-network-on-metamask-optional) chain on the MetaMask
network
- Import `wWGM` token (asset) on the WAGMI network (MetaMask). Here is the address - `0x3Ee7094DADda15810F191DD6AcF7E4FFa37571e4`
- `WGM` coins on the WAGMI chain. Drip `1 WGM` from the [WAGMI Faucet](https://faucet.metalblockchain.org/).
- `METAL` coins on the Fuji chain. Drip `10 METAL` from the [Tahoe Faucet](https://faucet.metalblockchain.org/)
...
```env
SRC_GATEWAY=https://swagmi.metalblockchain.org/wagmi/wagmi-chain-testnet/rpc
DST_GATEWAY=https://tahoe.metalblockchain.org/ext/bc/C/rpc

SRC_ADDR="<Your address on WAGMI>"
SRC_PK="<your private key on WAGMI>"
DST_ADDR="<Your address on Tahoe>"
DST_PK="<your private key on Tahoe>"
...
- `RESOURCE_ID` could be anything. It identifies our bridged ERC20 tokens on both sides (WAGMI and Tahoe).
...
relayer for relaying events from the WAGMI chain (source) to the Tahoe chain (destination).
...
**set to `100`. On Metal Mainnet, with this value, the proposals could be expired within 3-4 minutes.**
...
Run the following command to deploy Bridge, ERC20 Handler, and `wWGM` token contracts on the Tahoe
chain. Again it will set `DST_ADDR` as the default relayer for relaying events from Tahoe chain
