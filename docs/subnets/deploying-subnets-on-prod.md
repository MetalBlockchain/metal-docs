...
the design and testing it out on [the Tahoe Testnet](create-a-tahoe-subnet.md), eventually you will
...
Metal nodes are essential elements for running your Subnet in production. At a minimum, your
Subnet will need validator nodes, potentially also nodes that act as RPC servers, indexers or
explorers. Running a node is basically running an instance of [MetalGo](../nodes/README.md) on a
server.

### Server OS

Although MetalGo can run on a macOS or a Windows computer, we strongly recommend running nodes
...
For running MetalGo as a validator on the Primary Network the recommended configuration is as follows:
...
When installing the MetalGo node on the machines, unless you have a dedicated DevOps staff that
...
There are a number of different cloud providers.
...
Once you set up the server and install MetalGo on them, nodes will need to bootstrap (sync with
the network). This could be a lengthy process, as the nodes need to catch up and replay all the network
activity since the genesis up to the present moment.
...
tool to do that is [Subnet-CLI](https://github.com/MetalBlockchain/subnet-cli).
...
device with Metal Blockchain can be found
[here](/quickstart/ledger-app).
...
[the Subnet explorer](https://explorer.metalblockchain.org/subnets?tab=blockchains). Congrats!
...
`.metalgo/configs/node.json` for installer-script created nodes). A single node can sync
...
been through that on Testnet and Tahoe, but for a refresher, you can refer to [this
tutorial](create-a-tahoe-subnet.md).
...
```text
Jul 30 18:26:31 node-tahoe metalgo[1728308]: [07-30|18:26:31.422] INFO chains/manager.go:262 creating chain:
Jul 30 18:26:31 node-tahoe metalgo[1728308]:     ID: 2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt
Jul 30 18:26:31 node-tahoe metalgo[1728308]:     VMID:srEXiWaHuhNyGwPUi444Tu47ZEDwxTWrbQiuD7FmgSAQ6X7Dy
```
...
```text
Jul 30 18:27:21 node-fuji metalgo[1728308]: [07-30|18:27:21.055] INFO <2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt Chain> snowman/transitive.go:333 consensus starting with J5wjmotMCrM2DKxeBTBPfwgCPpvsjtuqWNozLog2TomTjSuGK as the last accepted block
```
...
[blockchain status](../apis/metalgo/apis/p-chain.md#platformgetblockchainstatus).

For a more in-depth look into Subnet operation, check out the blockchain log. By default, the log
can be found in `~/.metalgo/logs/ChainID.log` where you replace the `ChainID` with the actual ID
...
On Metal Blockchain all validations are limited in time and can range from two weeks up to one year.
...
If you have any questions, doubts or would like to chat, please check out our [Telegram Developer Chat](https://t.me/metaldevelopers)
