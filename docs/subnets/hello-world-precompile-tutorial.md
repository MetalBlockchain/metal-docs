...
[sha256 precompile](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/core/vm/contracts.go#L238-L252)
function.
...
not available in the default EVM, and are specific to Metal EVMs such as
[Coreth](https://github.com/MetalBlockchain/coreth) and [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm).

A stateful precompile follows this [interface](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/precompile/contract.go#L64-L67):
...
- Git Clone the [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm) repository
- Git Clone [AvalancheGo](https://github.com/MetalBlockchain/avalanchego) repository
- Install [Avalanche Network Runner](https://docs.metalblockchain.org/subnets/network-runner)
...
Please also use the [official GitHub tutorial](https://github.com/MetalBlockchain/hello-world-official-precompile-tutorial/pull/1)
...
```bash
cd $GOPATH/src/github.com/MetalBlockchain/subnet-evm
...
![Singleton StatefulPrecompiledContract and signatures](/img/2022-09-01-22-46-00.png)

We can paste it in
[`./precompile/params.go`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/precompile/params.go)
...
[`CALL`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/core/vm/evm.go#L222),
[`STATICCALL`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/core/vm/evm.go#L401),
[`DELEGATECALL`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/core/vm/evm.go#L362),
[`CALLCODE`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/core/vm/evm.go#L311),
it can [run the precompile](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/core/vm/evm.go#L271-L272)
if the precompile is enabled.

### Step 2: Set Gas Costs

In [`precompile/params.go`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/precompile/params.go)
...
Let's now modify
[`./params/precompile_config.go`](https://github.com/MetalBlockchain/subnet-evm/blob/HelloWorldOfficialTutorial/params/precompile_config.go).
...
[link](https://docs.metalblockchain.org/apis/avalanchego/apis/subnet-evm). The below code snippet can be
copied and pasted to overwrite the default code.
...
We can now add it to the `MetalRules()` which creates and returns a new instance of `Rules`. The
...
[this link](https://docs.MetalBlockchain/subnets/network-runner) for more information on Metal
...
latest MetalGo Binary.

In another terminal tab run this command in the root of the Subnet-EVM repository to get the latest local
Subnet-EVM binary. This build script will place the binary in `METALGO_PLUGIN_PATH` which we
will define later:
...
Leave the Subnet-EVM repository. Go to the MelalGo repository in whatever directory you keep
repositories and run the command below to get the latest MetalGo binary. The following commands
build and copy the binary to the `METALGO_EXEC_PATH` which we will define later:
...
We can now set the following paths. `METALGO_EXEC_PATH` points to the latest MetalGo binary
we have just built. `METALGO_PLUGIN_PATH` points to the plugins path which should have the
Subnet-EVM binary we have just built:

```bash
export METALGO_EXEC_PATH="${GOPATH}/src/github.com/MetalBlockchain/metalgo/build/metalgo"
export METALGO_PLUGIN_PATH="${GOPATH}/src/github.com/MetalBlockchain/metalgo/build/plugins"
```

The following command will "issue requests" to the server we just spun up. We can use
metal-network-runner to spin up some nodes that run the latest version of Subnet-EVM:
...
```bash
cd $GOPATH/src/github.com/MetalBlockchain/subnet-evm/contract-examples
...
[official tutorial](https://github.com/MetalBlockchain/hello-world-official-precompile-tutorial/pull/1)
to double check your work as well.
...
```bash
cp /tmp/subnet-evm-genesis.json $GOPATH/src/github.com/MetalBlockchain/subnet-evm/tests/e2e/genesis/hello_world.json
```
...
[Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm) and create a pull request.
