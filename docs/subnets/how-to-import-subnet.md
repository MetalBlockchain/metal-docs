# How to Import a Subnet into Metal-CLI

## Context

The most probable reason why someone would want to do this,
is if they already deployed a Subnet with Subnet-CLI either to Tahoe or Mainnet,
and now would like to use Metal-CLI to manage the Subnet.

Refer to the [installation guide](./install-metal-cli.md) for instruction about installing
Metal-CLI.

Because Subnet-CLI is now deprecated, you can use this tutorial to migrate to Metal-CLI.
...
but would now target Metal-CLI integration.

For this How-To, you import the WAGMI Subnet from Tahoe.
...
For these use cases, Metal-CLI now supports the `import public` command.
...
either the Tahoe testnet or Mainnet.
...
As stated earlier, this is from Tahoe, so select it.
As a next step, Metal-CLI asks for the path of the genesis file on disk:
...
Metal-CLI, or the node is obviously not reachable,
...
From the [Testnet Subnet Explorer](https://tahoe-explorer.metalblockchain.org/subnets?tab=blockchains)
...
```shell
Getting information from the Tahoe network...
...
Metal-CLI needs to know the VM type, to hit its repository and select
what VM versions are available.
This works automatically for Metal VMs (like Subnet-EVM and SpacesVM).
...
should know what they want to use Metal-CLI for, how,
...
This means you can now use Metal-CLI to handle the imported Subnet in the accustomed way.
