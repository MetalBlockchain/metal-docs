...
MetalGo provides the consensus engine for every blockchain on the Metal Network. The consensus
...
layer managed by Metal which deals with how nodes agree on whether or not to accept a block.

## Installing a VM

VMs are supplied as binaries to a node running `MetalGo`. These binaries must be named the VM's
assigned **VMID**. A VMID is a 32-byte hash encoded in CB58 that is generated when you build your VM.

In order to install a VM, its binary must be installed in the `MetalGo` plugin path. See
[here](../nodes/maintain/metalgo-config-flags.md#--build-dir-string) for more details.
Multiple VMs can be installed in this location.

Each VM runs as a separate process from MetalGo and communicates with `MetalGo` using gRPC
calls. This is functionality is enabled by `rpcchainvm`, a special VM that wraps around other VM
implementations so that they can communicate back and forth with the MetalGo.
...
initialized. The factory's `New` method shown below provides `MetalGo` with an instance of the
...
Before a VM can run, MetalGo will initialize it by invoking its `Initialize` method. Here, the
...
You can refer to the [implementation](https://github.com/MetalBlockchain/blobvm/blob/master/vm/vm.go#L92) of
...
To reach a consensus on linear blockchains, Metal uses the Snowman consensus engine. To be
compatible with Snowman, a VM must implement the `block.ChainVM` interface.

For more information, see [here](https://github.com/MetalBlockchain/metalgo/blob/master/snow/engine/snowman/block/vm.go).
...
For more information, you can see the full file [here](https://github.com/MetalBlockchain/metalgo/blob/master/snow/engine/common/vm.go).
...
For more information, you can see the full file [here](https://github.com/MetalBlockchain/metalgo/blob/master/snow/consensus/snowman/block.go).
...
For more information, you can see the full file [here](https://github.com/MetalBlockchain/metalgo/blob/master/snow/choices/decidable.go).
