...
MetalGo nodes support several different configuration files. Subnets have their own
...
The `subnet deploy` command deploys your Subnet configuration locally, to Tahoe Testnet, or to Mainnet.

At the end of the call, the command prints the RPC URL you can use to interact with the Subnet.

Metal-CLI only supports deploying an individual Subnet once per network. Subsequent
attempts to deploy the same Subnet to the same network (local, Tahoe, Mainnet) aren't
...
networks, so you can take your locally tested Subnet and deploy it on Tahoe or Mainnet.
...
The `network start` command starts a local, multi-node Metal network on your machine.
...
The `network status` command prints whether or not a local Metal network is running and
