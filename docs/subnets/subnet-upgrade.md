...
MetalGo node client. Same goes for the VM plugin binary that is used to run the blockchain on
your Subnet, which is most likely the [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm), the
...
When operating a Subnet, you should always keep in mind that Proof of Stake networks like Metal
...
equal to the amount of METAL staked on the node. On Subnets, weight is currently assigned by the
Subnet owners when they issue the transaction [adding a
validator](../apis/metalgo/apis/p-chain.md#platformaddsubnetvalidator) to the Subnet.
...
the value of
[`snow-sample-size`](../nodes/maintain/metalgo-config-flags.md#snow-sample-size-int) (default
...
latest MetalGo client immediately when it is made available.

For a general guide on upgrading MetalGo check out [this
tutorial](../nodes/maintain/upgrade-your-metalgo-node.md). When upgrading Subnet nodes and
...
API](../apis/metalgo/apis/health.md#healthhealth) to check that `healthy` value in the response
is `true` on the upgraded node, and on other Subnet validators check that
[platform.getCurrentValidators()](../apis/metalgo/apis/p-chain.md#platformgetcurrentvalidators)
...
Besides the MetalGo client itself, new versions get released for the VM binaries that run the
blockchains on the Subnet. On most Subnets, that is the
[Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm), so this tutorial will go through the steps for
...
In the future, VM upgrades will be handled by the [Metal-CLI
tool](https://github.com/MetalBlockchain/metal-cli), but for now we need to do it manually.

Go to the [releases page](https://github.com/MetalBlockchain/subnet-evm/releases) of the Subnet-EVM
...
metalgo`). You can monitor the log output on the node to check that everything is OK, or you can
use the
[info.getNodeVersion()](https://docs.metalblockchain.org/apis/metalgo/apis/info#infogetnodeversion) API
...
If you don't get the expected result, you can stop the `MetalGo`, examine and follow closely
step-by-step of the above. You are free to remove files under `~/metal-node/plugins`, however,
you should keep in mind that removing files is to remove an existing VM binary. You must put the
correct VM plugin in place before you restart MetalGo.
