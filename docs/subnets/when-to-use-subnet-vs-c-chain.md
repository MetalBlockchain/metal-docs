...
A Subnet is a subset of Metal Blockchain Primary Network validators agreeing to run the same [Virtual
...
[Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm)). The goal is to identify pros and cons of
building an app on C-Chain versus [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm), and help
...
but the gas must be paid in METAL. In the meantime,
[Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm) effectively creates an application-specific
EVM-chain with full control over native(gas) coins. The operator can pre-allocate the native tokens
in the chain genesis, and mint more using the [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm)
precompile contract. And these fees can be either burned (as METAL burns in C-Chain) or configured to
...
throughput. Plus, [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm) supports fee configuration
...
on-chain transactions. With [Subnet-EVM](https://github.com/MetalBlockchain/subnet-evm), an application can
...
The security of Metal Primary Network is a function of the security of the underlying validators
and stake delegators. Some choose C-Chain in order to achieve maximum security by utilizing Metal Primary Network validators. Some may choose to not rely on the entire
...
validate the Primary Network by staking METAL (minimum 2,000 METAL for Mainnet). For fault tolerance,
...
C-Chain is run and operated by nodes, it is highly decentralized and reliable, and all
...
infrastructure, and later expand to a Subnet. That way you can focus on working on the core of your project and once you have a solid product/market fit and have gained enough traction that the C-Chain is constricting you, plan a move to your own Subnet.

Of course, we're happy to talk to you about your architecture and help you choose the best path
forward. Feel free to reach out to us on [Telegram Developer Channel](https://t.me/metaldevelopers).
