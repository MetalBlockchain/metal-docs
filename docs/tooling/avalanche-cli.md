---
tags: [Tooling, Metal-CLI]
description: Metal-CLI is a command-line tool that gives developers access to everything Avalanche. This release specializes in helping developers build and test Subnets.
pagination_label: Metal-CLI
sidebar_position: 0
---

# Metal-CLI

Metal-CLI is a command-line tool that gives developers access to
everything Metal Blockchain. This release specializes in helping developers
...
[Install Metal-CLI](/tooling/cli-guides/install-metal-cli.md)
...
can be found [here](/reference/metalgo/p-chain/txn-format.md#proof-of-possession).

To get a node's BLS key and proof of possession key, call info.getNodeID API as shown [here](/reference/metalgo/info-api.md#infogetnodeid)

**Usage:**

```shell
metal primary addValidator [flags]
```
...
```shell
metal subnet removeValidator [subnetName] [flags]
```
...
```shell
  metal subnet changeOwner [subnetName] [flags]
```
...
[metal network clean](#network-clean) to reset all deployed chain state. Subsequent local
...
```shell
    --metalgo-path string      use this metalgo binary path
...
The `subnet import` command imports configurations into Metal-CLI.
...
```shell
metal subnet elastic [subnetName] [flags]
```
...
```shell
metal subnet join [subnetName] --elastic [flags]
```
...
```shell
metal subnet addPermissionlessDelegator [subnetName] [flags]
```
...
provide the `--metalgo-chain-config-dir` flag, this command attempts to write the upgrade file
...
```shell
metal subnet upgrade apply [subnetName] [flags]
```

**Flags:**

```shell
    --metalgo-chain-config-dir string   metalgo's chain config file directory (default "/Users/connor/.metalgo/chains")
...
```shell
metal subnet upgrade export [subnetName] [flags]
```
...
```shell
metal subnet upgrade generate [subnetName] [flags]
```
...
```shell
metal subnet upgrade import [subnetName] [flags]
```
...
```shell
metal subnet upgrade print [subnetName] [flags]
```
...
The validator will be validating the Metal Blockchain Primary Network and Subnet
...
```shell
  metal node create [clusterName] [flags]
```
...
```shell
    ...
    --metalgo-version-from-subnet string          install latest metalgo version, that is compatible with the given subnet, on node/s
    ...
    --custom-metalgo-version string               install given metalgo version on node/s
...
-h, --help                                            help for create
    --latest-metalgo-pre-release-version bool     install latest metalgo pre-release version on node/s
    --latest-metalgo-version bool                 install latest metalgo release version on node/s
...
```shell
  metal node destroy [clusterName] [flags]
```
...
```shell
  metal node devnet deploy [clusterName] [subnetName] [flags]
```
...
```shell
  metal node devnet wiz [clusterName] [subnetName] [flags]
```
...
-h, --help                                            help for wiz
    ...
    --node-config string                              path to metalgo node configuration for subnet
    --node-type string                                cloud instance type. Use 'default' to use recommended default instance type
    --num-validators ints                             number of nodes to create per region(s). Use comma to separate multiple numbers for each region in the same order as --region flag
...
```shell
  metal node export [clusterName] [flags]
...
the cluster owner. This will enable you to use `metal-cli` commands to manage the imported cluster.

Please note, that this imported cluster will be considered as EXTERNAL by `metal-cli`, so some commands
...
```shell
  metal node import [clusterName] [flags]
...
```shell
  metal node loadtest start [loadtestName] [clusterName] [subnetName] [flags]
```
...
```shell
  metal node loadtest stop [clusterName] [flags]
```
...
```shell
  metal node resize [clusterName] [flags]
...
```shell
  metal node ssh [clusterName] [flags]
```
...
If no cluster is given, defaults to node list behavior.
...
```shell
  metal node status [clusterName] [flags]
```
...
You can check the Subnet bootstrap status by calling metal `node status <clusterName> --subnet <subnetName>`

**Usage:**

```shell
  metal node sync [clusterName] [subnetName] [flags]
```
...
You can check the updated Subnet bootstrap status by calling metal
`node status <clusterName> --subnet <subnetName>`

**Usage:**

```shell
  metal node update subnet [clusterName] [subnetName] [flags]
```
...
```shell
  metal node validate primary [clusterName] [flags]
```
...
-m, --mainnet                   set up validator in mainnet
    --stake-amount uint         how many METAL to stake in the validator
...
```shell
  metal node validate subnet [clusterName] [subnetName] [flags]
```
...
Nodes created by `Metal-CLI` are protected by Cloud Security Group and only defined IP addresses
...
```shell
  metal node whitelist <clusterName> [--ip <IP>] [--ssh "<sshPubKey>"] [flags]
```
...
-n, --use-nano-metal   use nano METAL for balances
...
```shell
metal key transfer [options] [flags]
```

**Flags:**

```shell
-o, --amount float                 amount to send or receive (METAL units)
