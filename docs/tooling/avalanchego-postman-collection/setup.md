...
API calls that are available on an [AvalancheGo instance](https://github.com/MetalBlockchain/metalgo/releases/), including environment variables,
...
port from the outside by checking the appropriate [command line option](/nodes/configure/metalgo-config-flags.md#http-server).
